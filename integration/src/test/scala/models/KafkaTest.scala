...
class KafkaTest extends FunSuite with EmbdedKafkaCustom with EmbeddedKafka with BeforeAndAfterAll {
  ...
  test("should publish a message to a Kafka topic and receive it") {
  ...
  test("kafkaWithHeaders") {
    ...
    publishToKafkaWithHeaders("withHeaders", "", json.payload, meta.toStringMap.mapValues(_.getBytes))
...
trait EmbdedKafkaCustom {
  ...
  private def baseConsumerConfig(implicit config: EmbeddedKafkaConfig): Properties = {
    ...
    for ((k, v) ← config.customConsumerProperties)
      props.put(k, v)
